...
function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  const [, navigate] = useLocation();
  return <>{open && <button className="sidebar-scrim" onClick={onClose} aria-label="Close navigation" />}<aside className={`sidebar ${open ? "is-open" : ""}`}><div className="sidebar-head"><span className="eyebrow">THE COLLECTION</span><button onClick={onClose} className="close-button" aria-label="Close"><X size={20} /></button></div><p className="sidebar-intro">Explore the way to the answer.</p><div className="sidebar-section"><span className="sidebar-label">Problem collections</span>{catalog.map(item => <button key={item.path} className="catalog-link" onClick={() => { navigate(item.path); onClose(); }}><span>{item.label}</span><small>{item.count}</small><ChevronRight size={15} /></button>)}</div><div className="sidebar-section sidebar-note"><span className="sidebar-label">About this move</span><p>The Google Sites hierarchy is being reorganized into readable study pages while keeping the original problems and solutions.</p></div><div className="sidebar-footer"><BookOpen size={16} /><span>Math is a way of seeing.</span></div></aside></>;
}
